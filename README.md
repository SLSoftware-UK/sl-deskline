...
- **Supporting video**: an article can carry one optional YouTube video, shown between the summary and the body. Authors paste any YouTube link (watch, youtu.be, Shorts, embed or live) or a bare video ID into the editor's **YouTube video** field; only the 11-character ID is stored. **Show video as** picks an embedded player (privacy-enhanced `youtube-nocookie.com`, no cookies until play), a thumbnail linking to YouTube, or a "Watch on YouTube" button. The Content-Security-Policy allows just those two YouTube hosts (`frame-src www.youtube-nocookie.com`, `img-src i.ytimg.com`). Raw `<iframe>`s in the Markdown body are still stripped, so the field is the only way to add a video.
